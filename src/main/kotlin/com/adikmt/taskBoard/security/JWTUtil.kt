...
class JWTUtil {
    ...
    fun validateTokenAndReturnAuthentication(token: String): Mono<Authentication> {
        return Mono.justOrEmpty(token)
            .filter {
                isTokenExpired(it).not()
            }
            .flatMap(::extractClaims)
            .flatMap(::createUserAuthentication)
    }
    ...
    fun createUserAuthentication(claims: Claims): Mono<Authentication> {
        val subject = claims.subject
        return Mono.justOrEmpty(
            UsernamePasswordAuthenticationToken(UserPrincipal(subject), null)
        )
    }
